## Tests for the `StoreSessionHUD` autoload — the session-level owner of the
## persistent store_session HUD panels (right panel + event log).
##
## Covers the ownership / lifetime contract:
##   - `StoreSessionHUD._ready` spawns one `StoreStatusPanel` and one
##     `StoreEventLogPanel` as direct children; they live for the whole
##     process.
...
##     `StoreSessionState.day` and the active controller's `_objectives` for
##     passive milestones — idempotent across repeated calls.
##   - `StoreSessionHUD` does **not** subscribe to `EventBus.fp_mode_changed`; its
##     own session state is independent of FP mode.
extends GutTest


## Lightweight stand-in for `StoreSessionController` so the panel's
## controller-group lookup in `seed_for_day` finds a node carrying an
## `_objectives` array without dragging the full store_session scene into the test.
const _FAKE_CONTROLLER: GDScript = preload(
	"res://tests/gut/_fixtures/store_session_controller_stub.gd"
)
...
const _OBJECTIVES_DAY_1: Array[Dictionary] = [
	...
	},
	{
		"id": "stock_shelf",
		"stage": "stock_shelf",
		"label": "Day 1: Put a few items on the starter display table.",
...
func test_autoload_is_registered_and_owns_both_panels() -> void:
	assert_not_null(
		StoreSessionHUD,
		"StoreSessionHUD must be registered as an autoload (project.godot)"
	)
	assert_not_null(
		StoreSessionHUD.get_right_panel(),
		"StoreSessionHUD must own a StoreStatusPanel child"
	)
	assert_not_null(
		StoreSessionHUD.get_event_log_panel(),
		"StoreSessionHUD must own a StoreEventLogPanel child"
	)
...
func test_panels_are_direct_children_of_store_session_hud() -> void:
	...
	assert_eq(
		right.get_parent(), StoreSessionHUD,
		"StoreStatusPanel must be a direct child of the StoreSessionHUD autoload"
	)
	assert_eq(
		log.get_parent(), StoreSessionHUD,
		"StoreEventLogPanel must be a direct child of the StoreSessionHUD autoload"
	)
...
func test_activate_seeds_right_panel_header_from_day_argument() -> void:
	...
	StoreSessionHUD.activate(2)
	assert_true(
		StoreSessionHUD.get_right_panel().get_header_text().begins_with("DAY 2 —"),
		"Right panel header must reflect the activate(day) argument; got '%s'"
		% StoreSessionHUD.get_right_panel().get_header_text()
	)
...
func test_activate_seeds_objectives_from_active_controller() -> void:
	...
	StoreSessionHUD.activate(StoreSessionState.day)
	assert_eq(
		StoreSessionHUD.get_right_panel().get_visible_item_count(),
	...
	assert_eq(
		StoreSessionHUD.get_right_panel().get_item_glyph(first_id), "•",
...
func test_activate_after_day_started_already_fired_still_shows_day_n() -> void:
	...
	# `_ready` calls `StoreSessionHUD.activate(2)`. activate must force-seed so the
...
func test_seed_for_day_is_idempotent() -> void:
	...
	_install_stub_controller()
	StoreSessionHUD.activate(1)
	var header_after_first: String = StoreSessionHUD.get_right_panel().get_header_text()
	var count_after_first: int = StoreSessionHUD.get_right_panel().get_visible_item_count()
	var first_id: StringName = StringName(
		str(_OBJECTIVES_DAY_1[0].get("id", ""))
	)
	var glyph_after_first: String = StoreSessionHUD.get_right_panel().get_item_glyph(
		first_id
	)
	StoreSessionHUD.activate(1)
	assert_eq(
		StoreSessionHUD.get_right_panel().get_header_text(), header_after_first,
		"Repeated activate(1) must produce the same header text"
	)
	assert_eq(
		StoreSessionHUD.get_right_panel().get_visible_item_count(), count_after_first,
		"Repeated activate(1) must produce the same row count"
	)
	assert_eq(
		StoreSessionHUD.get_right_panel().get_item_glyph(first_id), glyph_after_first,
...
func test_storehud_does_not_connect_fp_mode_changed() -> void:
	# Scope clarification on the issue: StoreSessionHUD must NOT subscribe to
	...
	for entry: Dictionary in connections:
		...
		assert_ne(
			callable.get_object(), StoreSessionHUD,
			(
				"StoreSessionHUD must not connect to EventBus.fp_mode_changed; "
...
func test_fp_toggle_does_not_change_store_session_hud_active_state() -> void:
	...
	assert_true(
		StoreSessionHUD.is_active(),
		"StoreSessionHUD.is_active() must remain true through an FP-mode toggle"
	)
	EventBus.fp_mode_changed.emit(false)
	await get_tree().process_frame
	assert_true(
		StoreSessionHUD.is_active(),
		"StoreSessionHUD.is_active() must remain true after FP mode exits"
	)
...
func test_panels_survive_stub_controller_teardown() -> void:
	# Simulates the day-transition scenario: a `StoreSessionController`-shaped
	# node enters the tree, the autoload activates, the controller is freed,
	# and the panels are still alive and parented to StoreSessionHUD.
	var stub: Node = _install_stub_controller()
	StoreSessionHUD.activate(StoreSessionState.day)
	var right_before: StoreStatusPanel = StoreSessionHUD.get_right_panel()
	var log_before: StoreEventLogPanel = StoreSessionHUD.get_event_log_panel()
	stub.queue_free()
	await get_tree().process_frame
	assert_true(
		is_instance_valid(right_before),
		"StoreStatusPanel must outlive the day controller"
	)
	assert_true(
		is_instance_valid(log_before),
		"StoreEventLogPanel must outlive the day controller"
	)
	assert_eq(
		right_before.get_parent(), StoreSessionHUD,
		"StoreStatusPanel must remain parented to StoreSessionHUD after controller teardown"
	)
