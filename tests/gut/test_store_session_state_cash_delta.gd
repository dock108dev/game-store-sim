## StoreSessionState tracks per-day cash deltas, while EconomySystem owns the
## player-visible wallet. Day-summary accounting must therefore derive:
## ending_cash = round(EconomySystem.get_cash())
## starting_cash = ending_cash - StoreSessionState.daily_cash_delta.
## These tests guard that contract and the store_session delta reset behavior.
extends GutTest
...
func test_apply_decision_effect_accumulates_positive_cash_delta() -> void:
	StoreSessionState.apply_decision_effect(
