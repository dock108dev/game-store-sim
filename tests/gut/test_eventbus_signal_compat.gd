...
func test_issue_166_all_referenced_signals_are_declared() -> void:
	...
	assert_true(
		missing.is_empty(), "Referenced but undeclared EventBus signals:\n%s" % "\n".join(missing)
	)
...
func test_issue_166_emit_calls_match_declared_arity() -> void:
	...
	assert_true(
		mismatches.is_empty(), "EventBus emit arity mismatches:\n%s" % "\n".join(mismatches)
	)
...
func test_issue_166_audit_inventory_contains_connect_and_emit_calls() -> void:
	assert_gt(_connect_calls.size(), 0, "Expected connect call inventory in game/scripts")
	assert_gt(_emit_calls.size(), 0, "Expected emit call inventory in game/scripts")


func _parse_declared_signals(path: String) -> Dictionary:
	var text: String = FileAccess.get_file_as_string(path)
	var regex: RegEx = RegEx.new()
	var ok: Error = regex.compile("(?sm)^signal\\s+([A-Za-z0-9_]+)(?:\\s*\\((.*?)\\))?\\s*$")
	if ok != OK:
		return {}
...
func _scan_script(path: String) -> void:
	...
	for call: Dictionary in calls:
		...
		if call.get("method", "") == "emit":
			var args: Array[String] = _split_top_level_arguments(String(call.get("args", "")))
			(
				_emit_calls
				. append(
					{
...
func _record_connect_call(path: String, call: Dictionary, fn_ranges: Dictionary) -> void:
...
func _extract_function_arity_ranges(text: String) -> Dictionary:
	var regex: RegEx = RegEx.new()
	var ok: Error = regex.compile("(?m)^func\\s+([A-Za-z0-9_]+)\\s*\\(([^)]*)\\)")
	if ok != OK:
		return {}
...
func _extract_eventbus_member_calls(path: String, text: String) -> Array[Dictionary]:
	...
	while true:
		...
		if method_end <= method_start:
			cursor = method_start
			continue
		var method_name: String = text.substr(method_start, method_end - method_start)
		...
		if paren_close < 0:
			cursor = paren_open + 1
			continue
		var args_raw: String = text.substr(paren_open + 1, paren_close - paren_open - 1)
...
func _extract_emit_signal_calls(path: String, text: String) -> Array[Dictionary]:
	var result: Array[Dictionary] = []
	var cursor: int = 0
	while true:
		...
		if idx < 0:
			break
		var open_idx: int = _skip_spaces(text, idx + "EventBus.emit_signal".length())
		...
		if close_idx < 0:
			cursor = open_idx + 1
			continue
		var args_raw: String = text.substr(open_idx + 1, close_idx - open_idx - 1)
		var args: Array[String] = _split_top_level_arguments(args_raw)
		if args.is_empty():
			cursor = close_idx + 1
			continue
...
func _extract_connect_wrapper_calls(path: String, text: String) -> Array[Dictionary]:
	var result: Array[Dictionary] = []
	var regex: RegEx = RegEx.new()
	...
	for match: RegExMatch in regex.search_all(text):
		...
		if not first_arg.begins_with("EventBus."):
			continue
		var signal_name: String = _extract_identifier(first_arg.trim_prefix("EventBus."))
