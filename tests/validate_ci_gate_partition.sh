...
check_contains "$PR_WORKFLOW" "name: PR and Main Validation"
...
check_contains "$PR_WORKFLOW" "Unit Lane - Full GUT Tests"
...
check_contains "$PR_WORKFLOW" "sudo apt-get install -y xvfb mesa-utils libgl1-mesa-dri"
check_contains "$PR_WORKFLOW" "bash scripts/run_godot_tests.sh"
check_contains "$PR_WORKFLOW" "bash scripts/run_fresh_install_smoke.sh"
check_absent "$PR_WORKFLOW" "bash tests/automation/run_pr_smoke.sh"
...
check_contains "$NIGHTLY_WORKFLOW" "name: Weekly Scenario Review"
check_contains "$NIGHTLY_WORKFLOW" "17 8 * * 1"
...
check_absent "$NIGHTLY_WORKFLOW" "bash scripts/run_godot_tests.sh"
check_absent "$NIGHTLY_WORKFLOW" "bash scripts/run_fresh_install_smoke.sh"
check_absent "$NIGHTLY_WORKFLOW" "Static Lane - Repo Guards"
check_absent "$NIGHTLY_WORKFLOW" "Static Lane - GDScript Lint"
...
check_contains "$VIDEO_WORKFLOW" "name: Weekly Video Review"
check_contains "$VIDEO_WORKFLOW" "17 8 * * 1"
...
check_contains "$RELEASE_WORKFLOW" "sudo apt-get install -y xvfb mesa-utils libgl1-mesa-dri"
