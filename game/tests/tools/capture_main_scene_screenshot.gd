...
func _prepare_scenario(scene: Node, scenario: String) -> void:
	match scenario:
		...
		"lighting_materials_store":
			...
			_hide_node(scene, "SuspiciousCustomer")
			_hide_node(scene, "PlayerController")
			_set_camera(scene, Vector3(-0.55, 1.55, -4.92), Vector3(-2.85, 1.02, -0.7))
		"lighting_materials_mall":
			...
			_hide_node(scene, "SuspiciousCustomer")
			_hide_node(scene, "PlayerController")
			_set_camera(scene, Vector3(4.75, 1.52, -10.85), Vector3(0.25, 1.55, -5.95))
		"stocked_aisle":
			_prepare_customer_queue(scene)
			_hide_node(scene, "PlayerController")
		...
		"product_closeup":
			...
			_hide_node(scene, "SuspiciousCustomer")
			_hide_node(scene, "PlayerController")
			_set_camera(scene, Vector3(-1.52, 1.28, -4.82), Vector3(-0.5, 1.02, -4.25))
		"stockroom_doorway":
			_hide_node(scene, "CustomerManager")
			_hide_node(scene, "TradeInCustomer")
			_hide_node(scene, "PreorderCustomer")
			...
			_hide_node(scene, "SuspiciousCustomer")
			_hide_node(scene, "PlayerController")
			_set_camera(scene, Vector3(0.55, 1.42, 1.55), Vector3(0.2, 1.12, 3.65))
		"catalog_design_cues":
			_hide_node(scene, "PlayerController")
