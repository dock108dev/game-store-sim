...
func test_daily_report_formats_closed_day_totals() -> void:
	...
	assert_eq(report.get("closing_cash_cents"), 51038)
	...
	assert_eq(report.get("operating_expenses_cents"), 1000)
	assert_eq(report.get("reserved_obligations_cents"), 0)
	assert_eq(report.get("reputation_events"), 0)
	assert_eq(report.get("bills_text"), "daily overhead posted at close")
	assert_string_contains(str(report.get("cash_pressure_text")), "Daily overhead due at close: $10.00")
...
func test_daily_report_formats_launch_reputation_losses_and_tomorrow_plan() -> void:
	...
	session.record_reputation_event("launch_missed_neon_skyline_day_4", "Missed launch demand for Neon Skyline", "preorder", -10)
	session.end_day()
