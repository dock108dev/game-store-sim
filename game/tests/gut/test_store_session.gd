...
func test_store_session_exposes_owner_onboarding_baseline() -> void:
	...
	assert_string_contains(summary, "Next - Receiving: Bring incoming games from the stockroom receiving station")
...
func test_store_session_lists_available_supplier_lots() -> void:
	...
	assert_string_contains(session.get_supplier_order_summary_text(), "Storage: Receiving station intake")
	assert_string_contains(session.get_supplier_order_summary_text(), "Receiving station: Delivered as physical cases")
...
func test_store_session_formats_supplier_order_ui_details() -> void:
	...
	assert_eq(order.get("storage_requirement"), "Receiving station intake, then display rack or backstock shelf")
	assert_string_contains(str(order.get("receiving_expectation")), "physical cases in the backroom receiving station")
	...
	assert_string_contains(summary, "Storage needed: receiving station, then display rack or backstock shelf")
	assert_string_contains(summary, "Receiving expectation: physical cases appear in the backroom receiving station")
...
func test_store_session_orders_supplier_lot_and_reserves_cash() -> void:
	...
	assert_string_contains(session.get_supplier_order_summary_text(), "Used Game Starter Lot due to receiving station day 2 (3 cases)")
...
func test_store_session_delivers_supplier_lot_on_next_day() -> void:
	...
	assert_string_contains(session.get_supplier_order_summary_text(), "Used Game Starter Lot delivered to receiving station day 2")
	assert_string_contains(session.get_supplier_order_summary_text(), "Delivery state: delivered")
	assert_string_contains(session.get_supplier_order_summary_text(), "3 cases ready for pickup, pricing, and stocking")
	assert_string_contains(session.get_supplier_order_summary_text(), "Receiving workflow:")
	assert_string_contains(session.get_supplier_order_summary_text(), "Delivery point: Backroom receiving station / receiving_box_001")
	assert_string_contains(session.get_supplier_order_summary_text(), "World cues: open box, invoice clipboard, sorted tray")
	assert_string_contains(session.get_supplier_order_summary_text(), "Sort lanes: price, stock, backstock shelf")
...
func test_store_session_moves_receiving_items_to_backstock_and_retrieves_them() -> void:
	...
	assert_string_contains(session.get_storage_workflow_summary_text(), "Backstock shelf: Categorized bins / backstock_shelf_001")
