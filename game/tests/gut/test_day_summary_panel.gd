...
func test_day_summary_panel_opens_with_cash_and_sales_fields() -> void:
	...
	assert_string_contains(_panel.report_label.text, "Backroom report: day still open")
	...
	assert_string_contains(_panel.supplier_order_label.text, "Storage: Receiving station intake")
	assert_string_contains(_panel.supplier_order_label.text, "Receiving station: Delivered as physical cases")
	assert_string_contains(_panel.supplier_order_label.text, "Invoice: Count three cases")
	assert_string_contains(_panel.supplier_order_label.text, "Shelf plan: Price the strongest title")
	...
	assert_eq(_panel.order_games_button.text, "Order")
	assert_eq(_panel.open_box_button.text, "Open Box")
	assert_eq(_panel.check_invoice_button.text, "Invoice")
	...
	assert_eq(_panel.start_service_button.text, "Start Job")
	assert_eq(_panel.work_service_button.text, "Work Job")
	assert_eq(_panel.review_desk_button.text, "Review Desk")
	assert_eq(_panel.buy_upgrade_button.text, "Upgrade")
	...
	assert_eq(_panel.assign_category_button.text, "Category")
...
func test_day_summary_panel_switches_backroom_tab_visibility() -> void:
	...
	assert_true(_panel.set_active_tab("reports"))
	assert_eq(_panel.dashboard_header.text, "Reports")
	assert_true(_panel.report_label.visible)
	assert_false(_panel.summary_label.visible)
...
func test_day_summary_panel_commits_release_allocation_from_backroom_computer() -> void:
	...
	assert_lt(
		_panel.release_calendar_label.text.find("Release allocations:"),
		_panel.release_calendar_label.text.find("Release calendar:")
	)
...
func test_day_summary_panel_starts_launch_day_and_shows_release_outcome() -> void:
	...
	assert_lt(
		_panel.release_calendar_label.text.find("Launch events:"),
		_panel.release_calendar_label.text.find("Release calendar:")
	)
	assert_string_contains(_panel.report_label.text, "Launch events:")
	assert_string_contains(_panel.report_label.text, "Backroom report: day still open")
...
func test_day_summary_panel_orders_supplier_lot_from_backroom_computer() -> void:
	...
	assert_string_contains(_panel.supplier_order_label.text, "Used Game Starter Lot due to receiving station day 2 (3 cases)")
	...
	assert_eq(_panel.status_label.text, "Ordered Used Game Starter Lot to the backroom receiving station.")
...
func test_day_summary_panel_starts_next_day_and_delivers_supplier_lot() -> void:
	...
	assert_string_contains(_panel.supplier_order_label.text, "3 cases ready for pickup, pricing, and stocking")
	...
	assert_string_contains(_panel.status_label.text, "Opened Used Game Starter Lot at the receiving station.")
	...
	assert_string_contains(_panel.status_label.text, "Sorted Used Game Starter Lot onto the receiving station tray")
...
func test_day_summary_panel_moves_receiving_stock_to_backstock_and_back() -> void:
	...
	assert_string_contains(_panel.status_label.text, "Stored Star Trader on the backstock shelf.")
	assert_string_contains(_panel.fixture_label.text, "Backstock shelf: Categorized bins / backstock_shelf_001")
	assert_string_contains(_panel.fixture_label.text, "Pull stage: retrieve cases here before carrying them to sales-floor fixtures")
	...
	assert_string_contains(_panel.status_label.text, "Pulled Star Trader to the stockroom pull stage.")
