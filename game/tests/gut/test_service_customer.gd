...
func test_service_customer_describes_disc_resurfacing_request() -> void:
	...
	assert_string_contains(str(customer.call("interact")), "$4.99")
	assert_string_contains(str(customer.call("interact")), "$1.00")
...
func test_ledger_records_service_transaction() -> void:
	...
	assert_eq(transaction.get("service_price_cents"), 499)
	assert_eq(transaction.get("service_cost_cents"), 100)
	assert_eq(transaction.get("profit_cents"), 399)
	assert_eq(ledger.get_service_count(), 1)
	assert_eq(ledger.get_total_service_revenue_cents(), 499)
	assert_eq(ledger.get_total_service_cost_cents(), 100)
	assert_eq(ledger.get_total_service_profit_cents(), 399)
	assert_eq(ledger.get_total_revenue_cents(), 499)
	assert_eq(ledger.get_total_profit_cents(), 399)
