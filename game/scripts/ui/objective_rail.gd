...
## `_apply_modal_dim`) — the modal-fade contract keeps the
...
## interact with. In FP mode the rail can still render disabled focus copy
## without hiding the active objective label.
...
## visibility and the right-side chip immediately so FP focus copy does not
## wait for a fresh InteractionRay event.
...
##     (muted modulate + no badge for disabled focus). Cached HintLabel
##     remains the fallback when focus clears.
