...
## Fixed daily rent for the store_session loop. Surfaced on the day-summary panel
## under the Money section so the player sees today's sales offset by the
## day's operating cost. Constant for the store_session — a future tuning pass can
...
## EconomySystem-visible baseline instead of treating store_session deltas as a wallet.
...
func apply_decision_effect(
	...
	# get_cash() pipeline stays the single visible source of truth.
		...
		# pattern): callers can exercise the store_session ledger without a full
		# GameWorld in the tree. Production store_session path runs after Tier-1 init.
...
## store_session delta ledger and a headless-test fallback.
