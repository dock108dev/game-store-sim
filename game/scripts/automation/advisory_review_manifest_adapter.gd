...
static func _normalize_sweep_beats(manifest: Dictionary, artifact_dir: String) -> Array[Dictionary]:
	...
	for beat_variant: Variant in manifest.get("beats", []) as Array:
		...
		beats.append(_normalized_record({
			...
			"active_route_stage": str(beat.get("active_route_stage", "")),
			"local_action": str(beat.get("local_action", "")),
			"next_destination": str(beat.get("next_destination", "")),
			"visual_scope_mode": str(beat.get("visual_scope_mode", "")),
			"primary_work_surface_target": str(beat.get("primary_work_surface_target", "")),
...
static func _normalized_record(values: Dictionary) -> Dictionary:
	...
	return {
		...
		"active_route_stage": str(values.get("active_route_stage", "")),
		"local_action": str(values.get("local_action", "")),
		"next_destination": str(values.get("next_destination", "")),
		"visual_scope_mode": str(values.get("visual_scope_mode", "")),
		"primary_work_surface_target": str(values.get("primary_work_surface_target", "")),
