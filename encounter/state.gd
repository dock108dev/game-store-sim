...
func reset() -> bool:
 ...
 data = {"version":SCHEMA_VERSION,"clock":0.0,"queue":[],"decisions":[],"day":1,"orders":[],"phase":"prep","received":false,"cash":55000,"items":{},"customers":{},"sales":[],"carried":""}
 data.merge({"run_id":str(Time.get_unix_time_from_system())+"-"+str(Time.get_ticks_usec()),"ruleset_id":RULESET_ID,"opening_cash":55000,"layout":Layout.initial(),"layout_revision":0,"next_fixture_id":2,"next_event_seq":1,"fixture_events":[],"events":[],"fulfilled_requests":{}})
...
func valid_layout() -> bool:
 if data.get("ruleset_id")!=RULESET_ID or not data.get("run_id") is String or data.run_id.is_empty() or not data.run_id.is_valid_filename() or data.get("opening_cash")!=55000:return false
...
func valid() -> bool:
 ...
 if data.get("version")!=SCHEMA_VERSION or not whole(data.get("day")) or data.day<1 or data.day>7:return false
...
func load_from(path: String) -> bool:
 ...
 if parsed.get("version")!=SCHEMA_VERSION or parsed.get("ruleset_id")!=RULESET_ID:return storage_failed("load","incompatible")
