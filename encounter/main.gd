...
func _ready():
 ...
 if options.error!="":
  push_error(options.error);get_tree().quit(2);return
 ...
 if not capture and assortment_demo=="" and FileAccess.file_exists(session_file):
...
static func development_options(args: PackedStringArray, personal: bool) -> Dictionary:
 ...
 for arg in args:
  ...
  elif arg.begins_with("--pricing-demo="):options.error="Pricing comparison mode was retired. Use ordinary product labels and the current catalog."
...
func checkpoint_info() -> Dictionary:
 ...
 var probe=State.new()
 if not probe.load_from(save_path):
  if probe.last_storage_error.get("stage")=="incompatible":return {"ok":false,"text":"Incompatible checkpoint. Requires schema %d / %s. Original preserved; New week uses a separate file."%[State.SCHEMA_VERSION,State.RULESET_ID]}
  return {"ok":false,"text":"Unreadable, oversized, malformed or invalid checkpoint. Original preserved; New week uses a separate file."}
