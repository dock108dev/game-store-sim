...
func load_from(path: String) -> bool:
 ...
 if f == null:return storage_failed("load","open",FileAccess.get_open_error())
 var contents=f.get_as_text()
 var read_error=f.get_error()
 f.close()
 if read_error not in [OK,ERR_FILE_EOF]:return storage_failed("load","read",read_error)
 var parser=JSON.new()
 if parser.parse(contents)!=OK:return storage_failed("load","parse")
 var parsed=parser.data
 if not parsed is Dictionary:return storage_failed("load","shape")
