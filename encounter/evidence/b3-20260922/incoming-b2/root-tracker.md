...
Updated: 2026-09-22. Status: **PLAYABLE PROTOTYPE; B2 TECHNICALLY COMPLETE; OWNER ACCEPTANCE PENDING; B3 NEXT; NOT BETA READY.**

This is the entry point for the complete path to beta, replacing a tracker that ended at the next small owner review. The September 22 task authorized and implemented B2 runtime/UI changes and isolated verification. This does not authorize B3 or supply owner acceptance. The current R5 build remains available for focused feedback. A beta must deliver the agreed sustained shop experience, not merely pass the encounter checks.
...
| Persistent state | Schema 6 save/reload of inventory, fixture slots, expenses, cash, orders, customers and shift phases | Prototype checkpoint workflow; no autosave, backup rotation or prior-schema migration |
...
These are the required gaps between the current prototype and the confirmed first-week beta. B1 now specifies these systems in the [implementation packet](game-sim/docs/03-production/first-week-implementation-packet.md); B2 layout is implemented; the remaining systems below retain their assigned slices.
...
- **Shop development — REQUIRED:** grow and rearrange the shop over many days. B2 delivers editable fixture placement and four/eight-slot capacity; B3 must add meaningful growth progression. Keep growth within one shop. B2 delivers preparation-only rearrangement, valid walkable placement and the $60 second rack. B3 still owes the north-bay space/capacity upgrade.
...
| --- | --- |
| Three products, four/eight slots, three authored visitors | Suitable for a mechanic review; insufficient evidence of sustained variety or progression |
| Required systems are mostly absent | First-week scope is now defined, but growth, employees, rent/wages/bankruptcy, used condition/negotiation and releases still require implementation; B2 layout is now implemented |
...
The required features and first-week endpoint are confirmed; initial quantities and economic rules are now specified in the B1 packet and remain subject to B8 balancing. B1 and B2 are implemented within their documented scope. The sequence below governs the remaining work. Integrate each feature into the same shop/save/economy rather than creating disconnected demos. Relevant art, controls and saves are part of every stage.
...
- [Launch current B2 development build](game-sim/encounter/Launch%20Encounter.command). No running window or fresh owner session is claimed by this planning update.
...
The next engineering task is **B3 — Growth and business progression**, after separate authorization. Use [B2 delivery and handoff](game-sim/docs/03-production/b2-delivery.md) and the B1 packet. Extend schema 6 / b2-layout-1 into B3’s schema 7 namespace; preserve the B2 source/evidence and the owner’s R5 saves. B3 owns the north bay, included rack-0003, $100 expansion, bills, insolvency and seven-day result. Reuse layout validation, stable slots and paid event identities; actual staff stay B4 and releases/order-timing changes stay B6.

B2 stops here. Technical checks do not supply B2 or R5 owner acceptance. Worker contention remains B4; full-week balance remains B8; motion remains B7; packaged Mac launch and the historical exit warning remain B9; B10 requires a separate complete-week owner decision.
