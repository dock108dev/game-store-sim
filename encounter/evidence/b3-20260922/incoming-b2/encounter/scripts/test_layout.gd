...
func _initialize():
 var s=State.new()
 check("fresh schema6 four slots zero expense and original economy",s.valid() and s.data.version==6 and s.capacity()==4 and s.data.cash==55000 and s.data.fixture_events.is_empty() and State.PRODUCTS.size()==3)
 s.receive()
 check("three uniquely acquired starter copies",s.data.items.size()==3 and s.valid())
 ...
 check("eight-slot boundary and remaining backroom",s.shelf_used()==8 and s.count_at("backroom")==3 and not s.stock("orbit",9) and s.valid())
 ...
 finish(s)
 # Spend down cash with real supplier commands, never tamper cash to simulate poverty.
 while s.data.cash>=6000:
  if not s.pending_shipment().is_empty():break
  var quantities={"curb":6,"tide":0,"orbit":0}
  if not s.order(quantities,s.data.day):break
  s.advance(s.data.day);s.receive();s.open();finish(s)
 ...
 while poor.data.cash>=6000:
  poor.open();finish(poor)
  var q=mini(6,int(poor.data.cash/1200));poor.order({"curb":0,"tide":q,"orbit":0},poor.data.day);poor.advance(poor.data.day);poor.receive()
