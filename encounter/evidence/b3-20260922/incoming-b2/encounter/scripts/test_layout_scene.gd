...
func run():
 ...
 await click_at(Vector2(310,450));await settle()
 check("advance receives same six copies on day2",scene.state.data.day==2 and scene.state.count_at("backroom")==6 and scene.state.capacity()==8 and scene.state.valid())
