...
func _initialize():
 ...
 check("receive starter once",s.receive() and not s.receive() and s.data.items.size()==6)
 ...
 check("backroom allowed while open",s.open() and s.count_at("backroom")==4)
 check("no prep changes while trading",not s.unstock("curb") and not s.reprice(100,"curb"))
 ...
 check("receive mixed once",s.receive() and not s.receive() and s.data.items.size()==12)
 s=rt(s,"mixed received")
 for p in State.PRODUCTS:s.reprice(1000,p)
 check("capacity four and overflow",s.stock("curb",2) and s.stock("tide",6) and not s.stock("orbit") and s.shelf_used()==4 and s.count_at("backroom")==6)
 var costs={}
 ...
 for id in s.buyer_ids():check("day2 sought product buys "+id,select(s,id) and s.queue(id) and pay(s,id))
