...
func run():
 ...
 scene=load("res://main.tscn").instantiate();root.add_child(scene);scene.save_path="user://b6-scene.json";scene.state.checkpoint_path=scene.save_path;await frames(5)
