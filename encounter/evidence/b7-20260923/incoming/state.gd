...
func save_to(path: String) -> bool:
 ...
 if FileAccess.file_exists(path):
  ...
  if previous.parse(FileAccess.get_file_as_string(path))!=OK or not previous.data is Dictionary or previous.data.get("version")!=10 or previous.data.get("ruleset_id")!="b6-retail-1":return false
 ...
 f.close()
 ok=ok and DirAccess.rename_absolute(path+".tmp",path) == OK
 if ok:last_checkpoint_label="Day %d • %s"%[data.day,data.phase.replace("_"," ")]
 return ok
...
func checkpoint(_reason: String) -> bool:
 save_pending=true
 if checkpoint_path.is_empty():return false
 return retry_save()
func retry_save() -> bool:
 last_save_ok=save_to(checkpoint_path) if not checkpoint_path.is_empty() else false
...
func restart(path: String) -> bool:
 if data.phase in ["week_complete","bankrupt"]:
  ...
  if FileAccess.file_exists(archive):
   ...
   if not retained.load_from(archive) or retained.data!=data:return false
