...
func _ready():
 ...
 button("Restart week",Vector2(944,590),288,confirm_reset)
 ...
 if saved_probe.load_from(save_path):state.last_checkpoint_label=saved_probe.last_checkpoint_label
...
func confirm_reset():
 var dialog=ConfirmationDialog.new();dialog.theme=GlassUI.make_theme();dialog.dialog_text="Start a new seven-day business?\nA completed or bankrupt run must be preserved before restart.\nOther unsaved progress will be discarded.";dialog.title="Restart week";dialog.confirmed.connect(func():perform("reset"));dialog.confirmed.connect(dialog.queue_free);dialog.canceled.connect(dialog.queue_free);ui.add_child(dialog);dialog.popup_centered(Vector2i(650,180))
...
func show_daily_reports():
 ...
 for row in state.data.settlements:
  var r=row.report
  rows.append("Day %d • Cash %s • Margin %s • Overhead %s • Unpaid %s"%[row.day,money(r.cash),money(r.margin),money(r.incurred_overhead),money(r.unpaid_liabilities)])
...
func refresh():
 ...
 bills_label.position.y=64 if state.data.layout.space_level==1 else 144
 ...
 expand_button.visible=draft.is_empty()
 ...
 shelf_label.position=Vector2(42,86) if state.data.layout.space_level==1 else Vector2(52,224)
 if shelf_panel!=null:shelf_panel.visible=state.data.layout.space_level==0
 for control in [arrange_button,buy_button,rack_select,expand_button,confirm_button,cancel_button]:control.position.y=586 if state.data.layout.space_level==1 else 170
 day_label.visible=state.data.layout.space_level==0
 ...
 if state.data.phase in ["week_complete","bankrupt"]:
  ...
  guide.text="First week complete • Review finances or restart." if state.data.phase=="week_complete" else "Bankrupt • A due bill could not be paid. Review liabilities or restart."
 ...
 if state.data.phase=="report":
  ...
  details.tooltip_text="Fixtures / expansion paid: "+money(r.investment)+". Revenue: today’s completed sales. Inventory cost: historical purchase cost of each sold copy. Gross profit: revenue minus that cost, before overhead. Orders reduce cash, not profit, until sold."
...
func show_staff():
 ...
 for id in State.Staff.IDS:
  ...
  var title=Label.new();title.custom_minimum_size.x=250;title.text=id.capitalize()+" · "+state.data.staff[id].role;title.tooltip_text=workers[id].status;row.add_child(title)
...
func bodies() -> Array:
 ...
 return result
func dynamic_route(actor,target: Vector2) -> Dictionary:
 var cells=[]
 for other in bodies():
...
func show_seller():
 ...
 info.text="%s · USED / %s\nSuggested resale %s · Asking %s\nOne initial offer, one seller counter, at most one final offer.\nPurchased copies need a label next preparation."%[State.CATALOG[row.product].name,row.condition.capitalize(),money(row.resale),money(row.asking)]
...
func yield_to_queue(actor) -> bool:
 # A stable right of way prevents two automatic routes from chasing each other.
 # Keep the lower-priority body still; the buyer routes around its real position.
 for id in state.data.customers:
  var c=state.data.customers[id]
  if (c.state=="leaving" or (c.state=="queued" and not c.settled)) and visitors.has(id):
   var buyer=visitors[id].actor
   if buyer.visible and actor.position.distance_to(buyer.position)<90:return true
 return false
