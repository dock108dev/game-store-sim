...
func _ready():
 ...
 var options=development_options(OS.get_cmdline_user_args(),OS.has_feature("personal_beta"))
 assortment_demo=options.assortment
 if assortment_demo!="":save_path="user://assortment-"+assortment_demo+".json"
 if options.pricing!="":
  demo_price={"low":1699,"reference":2199,"high":2699}[options.pricing]
  save_path="user://pricing-demo-"+options.pricing+".json"
 ...
 if not capture and assortment_demo=="" and options.pricing=="" and FileAccess.file_exists(session_file):
  var locator=FileAccess.open(session_file,FileAccess.READ)
  var active=""
  if locator!=null:
   if locator.get_length()<=256:active=locator.get_buffer(256).get_string_from_utf8().strip_edges()
   locator.close()
...
static func development_options(args: PackedStringArray, personal: bool) -> Dictionary:
 var options={"assortment":"","pricing":"","capture":false}
 # The personal export never consumes fixture/capture switches, even on debug builds.
 if personal:return options
 for arg in args:
  if arg in ["--assortment-demo=stocked","--assortment-demo=missing"]:options.assortment=arg.get_slice("=",1)
  elif arg in ["--pricing-demo=low","--pricing-demo=reference","--pricing-demo=high"]:options.pricing=arg.get_slice("=",1)
  elif arg=="--capture":options.capture=true
 return options
func checkpoint_info() -> Dictionary:
 ...
 var probe=State.new()
 var result=probe.read_checkpoint(save_path)
 if not result.ok:return {"ok":false,"text":"Unreadable, oversized or malformed checkpoint. The original file is preserved; New week will use a separate file."}
 if result.data.get("version")!=10 or result.data.get("ruleset_id")!="b6-retail-1":return {"ok":false,"text":"Incompatible checkpoint. Requires schema 10 / B6 retail rules. Original preserved; New week uses a separate file."}
