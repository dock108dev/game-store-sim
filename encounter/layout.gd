...
static func floor_bounds(space_level=0) -> Rect2:return Rect2(180,220,720,390) if space_level==1 else Rect2(180,300,720,310)
...
static func ports(layout: Dictionary,id: String) -> Dictionary:
 ...
 match f.type:
  ...
  "counter":return {"cashier":p+Vector2(-30,30),"buyer":p+Vector2(110,100),"intake":p+Vector2(160,-50),"inspect":p+Vector2(120,-50)}
 return {}
...
static func walkable(layout: Dictionary,p: Vector2) -> bool:
 if not floor_bounds(layout.space_level).has_point(p):return false
 for b in obstacles(layout):
  if b.grow(RADIUS).has_point(p):return false
 return true
static func navigation(layout: Dictionary) -> AStarGrid2D:
 var grid=AStarGrid2D.new();grid.region=Rect2i(18,22 if layout.space_level==1 else 30,72,39 if layout.space_level==1 else 31);grid.cell_size=Vector2(10,10);grid.diagonal_mode=AStarGrid2D.DIAGONAL_MODE_NEVER;grid.update()
 for x in range(18,90):
  for y in range(22 if layout.space_level==1 else 30,61):
...
static func check(layout) -> Dictionary:
 if not layout is Dictionary or (layout.get("space_level")!=0 and layout.get("space_level")!=1) or not layout.get("fixtures") is Dictionary:return failure("Unsupported shop space")
 var fixtures=layout.fixtures
 if fixtures.has("rack-0003")!=(layout.space_level==1):return failure("Expansion rack missing or incompatible")
 if fixtures.size()<3 or fixtures.size()>(5 if layout.space_level==1 else 4):return failure("Rack limit reached")
 for id in fixtures:
  var f=fixtures[id]
  ...
  if id not in ["rack-0001","rack-0002","rack-0003","receiving-01","checkout-01"]:return failure("Unknown fixture",id)
  if id.begins_with("rack") and f.type!="rack":return failure("Invalid rack",id)
  var b=footprint(f)
  if not floor_bounds(layout.space_level).encloses(b):return failure("Outside shop",id)
  if b.intersects(Rect2(190,220,60,390)) or b.intersects(Rect2(180,560,720,50)):return failure("Blocks reserved aisle",id)
