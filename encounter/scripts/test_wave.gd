...
func _initialize():
 ...
 check("fixed identities and individual budgets",s.data.customers.keys()==State.VISITORS and s.data.customers["visitor-1"].budget==2418 and s.data.customers["visitor-2"].budget==1759 and s.data.customers["visitor-3"].budget==3078)
 ...
 for id in State.VISITORS:check("no stock distinct miss "+id,not select(s,id) and s.data.customers[id].decision=="unavailable")
 ...
 s=prep(2199)
 for id in State.VISITORS:select(s,id)
