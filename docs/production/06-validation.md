...
- Suspicious event flags do not visibly interrupt normal store progression.
...
- `scenarios/hidden_thread.json`: hidden suspicious event log existence, flag recording, deduplication, input normalization, optional mismatched serial checks, optional supplier message checks, and optional evidence storage checks.
