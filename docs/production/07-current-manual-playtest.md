...
Run these first when manually checking the mechanically complete employees-only stockroom implementation:

- Review `docs/production/17-stockroom-production-plan.md` and confirm Slices 1-8 keep the backroom physical rather than adding another abstract inventory menu.
...
- For storefront work, compare `main_scene.png` and `storefront_entry.png` against `inspiration/IMG_1033.PNG`, `IMG_1035.PNG`, `IMG_1038.PNG`, `IMG_1054.PNG`, and `IMG_1055.PNG`; pass only if the first view reads as a real small game shop with a bounded readable `SAVE POINT GAMES` sign and the window display reads as a curated shelf with used/sealed cases, small hardware, and display lighting rather than loose blocks.
...
- For sales-floor fixture work, confirm `main_scene.png`, `stocked_aisle.png`, and `customer_queue.png` show used games, new releases/preorders, accessories, services/trade-ins, reusable rack spines, and `USED`/`STAFF` shelf talkers without crowding interaction targets.
- For register work, confirm `register_counter.png` reads as a command center with scanner/payment/receipt/cash/return/trade-in/preorder/service cues, including the new counter-top work rail and short `PAY`, `RECEIPT`, `RETURNS`, `TRADE`, `PRE`, and `SVC` labels, while preserving the single register interaction target.
- For product visual work, confirm `carry_stack.png`, `receiving_area.png`, and shelf views make cases/cards/accessories look like inventory rather than blocks, with backed product-facing tag cards, category stripes, pin details, and price/condition/risk text readable only at appropriate distance.
